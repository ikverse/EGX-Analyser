...
import { AiProvider, ApiClient, Channel, Consensus, ContentUpdateStatus, DiagnosticEntry, SelectedAnalysisResult, SettingsInput, SettingsStatus, TelegramChat } from "./api";
...
function Channels({ channels, api, refresh, notify }: { channels: Channel[]; api: ApiClient; refresh: () => Promise<boolean>; notify: Notify }) {
  ...
  const analyze = () => { const ids = selectedChannels.map((channel) => channel.id); if (!ids.length) return notify("warning", "Select at least one chat first."); setLoading(true); void api.analyzeSelected(ids, lookbackDays).then((result) => { setLastAnalysis(result); return refresh().then(() => notify(result.not_stock_related.length ? "warning" : "success", `${result.messages_analyzed} of ${result.messages_in_window} messages were freshly analyzed from the last ${result.lookback_days} day(s) (${result.messages_reanalyzed} re-analyzed). Report and local trace created.${result.not_stock_related.length ? ` No stock-related context: ${result.not_stock_related.join(", ")}.` : ""}`)); }).catch((reason) => notify("error", displayError(reason))).finally(() => setLoading(false)); };
...
function CloudSettings({ api, status, onSaved, notify, checkingUpdate, onCheckForUpdates }: { api: ApiClient; status: SettingsStatus | null; onSaved: () => Promise<boolean>; notify: Notify; checkingUpdate: boolean; onCheckForUpdates: () => void }) {
  const [values, setValues] = useState<SettingsInput>({ ai_provider: status?.ai_provider || "qwen", openai_model: status?.openai_model || "qwen3-vl-plus", analysis_instructions: status?.analysis_instructions || "" });
  ...
  useEffect(() => { if (status) setValues((current) => ({ ...current, ai_provider: status.ai_provider, openai_model: status.openai_model, analysis_instructions: status.analysis_instructions })); }, [status]);
  const save = (event: FormEvent) => {
    event.preventDefault();
    void api.saveSettings(values).then(onSaved).then(() => {
      setValues((current) => ({ ai_provider: current.ai_provider, openai_model: current.openai_model, analysis_instructions: current.analysis_instructions }));
  ...
  return <div className="settings">
    <form onSubmit={save}>
      ...
      <label>Primary analysis prompt<textarea value={values.analysis_instructions || ""} onChange={(event) => setValues((current) => ({ ...current, analysis_instructions: event.target.value }))} placeholder="For example: prioritize EGX table rows, show entry and targets exactly as posted, and flag conflicting channel details." rows={6} /><span className="credential-note">When filled, this replaces the built-in analysis prompt for every request. Leave it empty to use the built-in prompt. The required structured output format remains enforced.</span></label>
