...
export default function App() {
  ...
  const installUpdate = async () => {
    ...
    try {
      ...
      notify("error", `Update could not be installed: ${displayError(reason)}`);
    }
  };
...
function Reports({ api, rows, setRows, notify }: { api: ApiClient; rows: Array<Record<string, unknown>>; setRows: (rows: Array<Record<string, unknown>>) => void; notify: Notify }) {
  return <><button onClick={() => void api.generateReport().then(() => api.reports()).then(setRows).then(() => notify("success", "Daily report created.")).catch((reason) => notify("error", displayError(reason)))}>Generate daily report</button><Table rows={rows} /></>;
}
...
function Settings({ api, status, onSaved, notify, checkingUpdate, onCheckForUpdates }: { api: ApiClient; status: SettingsStatus | null; onSaved: () => Promise<void>; notify: Notify; checkingUpdate: boolean; onCheckForUpdates: () => void }) {
  ...
  return <div className="settings">
    ...
    <form onSubmit={(event) => { event.preventDefault(); void api.requestTelegramCode(phone).then(() => notify("success", "Telegram code sent. Enter it below.")).catch((reason) => notify("error", `Could not send Telegram code: ${displayError(reason)}`)); }}><h3>Connect Telegram {status?.telegram_authorized ? "✓" : ""}</h3><label>Phone number<input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+201..." required /></label><button>Send code</button></form>
    <form onSubmit={(event) => { event.preventDefault(); void api.verifyTelegramCode(code, password || undefined).then(() => onSaved()).then(() => notify("success", "Telegram connected and saved for future launches.")).catch((reason) => notify("error", `Telegram connection failed: ${displayError(reason)}`)); }}><label>Verification code<input value={code} onChange={(event) => setCode(event.target.value)} required /></label><label>Two-step password (only if enabled)<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} /></label><button>Verify code</button></form>
  </div>;
}
