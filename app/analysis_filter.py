...
def is_non_actionable_stock_update(text: str) -> bool:
    """Identify historical recommendation updates that must not reach the model."""
    normalized = _normalize_arabic(text)
    return has_past_recommendation_context(text) or any(
        _normalize_arabic(marker) in normalized for marker in _TARGET_HIT_MARKERS
    )
